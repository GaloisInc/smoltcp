...
fn main() {
    ...
    let udp1_rx_buffer = UdpSocketBuffer::new(vec![UdpPacketMetadata::EMPTY], vec![0; 6000]);
    let udp1_tx_buffer = UdpSocketBuffer::new(vec![UdpPacketMetadata::EMPTY], vec![0; 6000]);
    let udp1_socket = UdpSocket::new(udp1_rx_buffer, udp1_tx_buffer);

    let udp2_rx_buffer = UdpSocketBuffer::new(vec![UdpPacketMetadata::EMPTY], vec![0; 6000]);
    let udp2_tx_buffer = UdpSocketBuffer::new(vec![UdpPacketMetadata::EMPTY], vec![0; 6000]);
    ...
    loop {
        ...
        match iface.poll(&mut sockets, timestamp) {
            Ok(_) => {},
            Err(e) => {println!("server poll error: {}",e);}
        };
